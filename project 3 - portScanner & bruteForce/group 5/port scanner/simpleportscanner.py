...
os.system('reset') # Clear the console window
...
close_ports = [] # The list is used to hold the closed ports
...
try:
	...
	if flag: # The flag is set, means the user did not give any port range
		for p in sorted(common_ports): # So we will scan the common ports. 
			...
			if response == 0: # The port is open
				...
				close_ports.append(p) # append it to the list of closed ports
			#if not p == end_port:
	...
	if open_ports: # There are open ports available
		...
		for i in sorted(open_ports):
			...
			print colored("\t%s %s: Open" % (i, service), 'green')

	if close_ports: # There are open ports available
		print "Close Ports: "
		for i in sorted(close_ports):
			service = get_service(i)
			if not service: # The service is not in the disctionary
				service = "Unknown service"
			#print "\t%s %s: Close" % (i, service)
			print colored("\t%s %s: Close" % (i, service), 'red')


	if not open_ports or not close_ports:
